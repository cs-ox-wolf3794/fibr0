...
- **Measured per-event cost on the first live batch: USD 0.016** (8 events, USD 0.127 total, 14k cached input tokens, 1.2k output tokens per event). The earlier estimate of USD 0.055 was 3.5x too high because prompt caching on the system prompt works inside batches and the model writes short output for non-events. The cap is 15 per run on that basis.
- **First live run lesson:** ordering by source tier first sent six routine items to the model (buyback disclosures, a trade-show release, EDGAR index lines with no body, a generic weather outlook) and it correctly produced zero impacts for all of them, while the five-source Saudi pipeline story was skipped. Ordering is now corroboration first. The model's refusal to invent impacts for non-events is a feature; never prompt it toward producing at least one impact.
- Publish reads from `llm_outputs`, not from the analyze stage's return value, and marks `events.published_at` for every event it handles. Running `--stage analyze` alone is therefore safe; a later `--stage publish` picks the results up.
- The CLI commits after every stage. Before that fix, the analyze stage held an open transaction for the whole Batch API wait, which blocked an `ALTER TABLE` until Supabase's statement timeout cancelled it. Never run a migration while a pipeline process is mid-batch.
- EDGAR is disabled until there is a fetcher for the filing body. The atom index carries only "8-K - Company (CIK)" and the model cannot do anything with that.
